...
Record first. Interpret later. Compare across 3, 7, 14, and 28 days.`;
  ...
  function buildMirrorReading() {
    ...
    const focus = val("readingFocus") || "general";
    const recent = logs().slice(0, 7);
    
    // Extract recent patterns
    const recentWords = new Set();
    const recentSigns = new Set();
    const recentBodyNotes = new Set();
    
    recent.forEach(log => {
      if (log.signs) log.signs.split(/[,\s]+/).forEach(w => w && recentWords.add(w.toLowerCase()));
      if (log.body) log.body.split(/[,\s]+/).forEach(w => w && recentBodyNotes.add(w.toLowerCase()));
    });

    const focusFrames = {
      general: {
        opening: "The day mirrors the witness. Look at what actually happened before explaining it.",
        signal: `Today is a ${dayArch[0]} day within ${info.moon.name}. The visible moon is ${phase} (${illum}%). Your solar gate is ${solar[0]}.`,
        watch: recentWords.size > 0 
          ? `Recent repeating words: ${Array.from(recentWords).slice(0, 3).join(", ")}.`
          : "Track what words, numbers, or themes repeat across multiple days.",
        shadow: "The shadow is explaining the day before the pattern has repeated enough to become clear.",
        action: "Record what happened. Wait for the pattern to show itself across 3 or more days."
      },
      family: {
        opening: "For family and custody: see the child first, the protection second, the order third.",
        signal: `Today is a ${dayArch[0]} day. ${info.moon.name} emphasizes ${info.moon.essence.toLowerCase()}. How does this show in your family's day?`,
        watch: "Watch: sleep patterns, emotional shifts, communication clarity, safety concerns.",
        shadow: "The shadow is reacting from old pain instead of staying present to what the child needs now.",
        action: "Document one clear fact. Speak one clear boundary. Take one action that builds stability."
      },
      work: {
        opening: "For work and provision: repair the system, not the crisis.",
        signal: `Today is a ${dayArch[0]} day. ${info.moon.name} carries ${info.moon.freq}. What one thing needs fixing in your work or provision?`,
        watch: "Watch: delays, communications, resources, timing, team signals.",
        shadow: "The shadow is panic, scattering, or trying to finish everything at once.",
        action: "Choose one repair. Complete it cleanly. Let order build."
      },
      body: {
        opening: "For body and health: feel first, interpret later.",
        signal: `Today is a ${dayArch[0]} day. What is your body saying? The moon is ${phase}. ${recentBodyNotes.size > 0 ? `Recent body notes: ${Array.from(recentBodyNotes).slice(0, 3).join(", ")}.` : ""}`,
        watch: "Watch: sleep, pressure, hunger, clarity, pain, energy, rest.",
        shadow: "The shadow is ignoring the body's signal while trying to push through.",
        action: "Give the body what it asks for: food, water, rest, or movement. Let the body teach."
      },
      spiritual: {
        opening: "For prayer and spiritual work: listen more than you speak.",
        signal: `Today is a ${dayArch[0]} day. ${info.moon.name} is calling for ${info.moon.practice.toLowerCase()}. What is the current spiritual work?`,
        watch: "Watch: timing, impulse, clarity, humility, mercy, witness.",
        shadow: "The shadow is confusing intensity or excitement with spiritual instruction.",
        action: "Return to prayer, patience, and waiting. Let the fruit become visible."
      },
      creative: {
        opening: "For creative and Codex work: build one layer at a time.",
        signal: `Today is a ${dayArch[0]} day within ${info.moon.name}. What is the next layer of the Codex?`,
        watch: "Watch: language, symbols, timing, clarity, resonance.",
        shadow: "The shadow is trying to complete the whole work before the foundation is clear.",
        action: "Write one clean piece. Let the work become coherent page by page."
      }
    };

    const frame = focusFrames[focus] || focusFrames.general;

    const reading =
`☲ REMNANT MIRROR READING ☲

${fmtDate(selectedDate)} · ${focus.charAt(0).toUpperCase() + focus.slice(1)} Focus
...
Visible Moon Phase: ${phase} · ${illum}% illuminated
...
Mirror Questions for Today

1. What actually happened, without explanation?
2. What did my body know?
3. What repeated from recent days?
4. What is asking for repair, not reaction?
5. What one grounded step can I take now?

━━━━━━━━━━━━━━━━━

Closing Words

The calendar does not predict or command. It reflects the day back to the witness. 
Record first. Interpret after the pattern repeats across 3 or more days. 
Move with wisdom. The calendar is the record of the patterns. You are the witness.`;

    text("mirrorTitle", `${info.moon.name} · ${dayArch[0]} Mirror`);
    text("mirrorOutput", reading);
    
    // Update compass cards
    text("mirrorSignal", frame.signal);
    text("mirrorShadow", frame.shadow);
    text("mirrorAction", frame.action);
  ...
  function render() {
    ...
    if (weekDots) {
      weekDots.innerHTML = Array.from({ length: 28 }, (_, i) => {
        const n = i + 1;
        const cls = !info.inside ? "" : n < info.dayInMoon ? "active" : n === info.dayInMoon ? "active" : "";
  ...
  function setupTabs() {
    ...
    // Tab jumping with data-tab-jump
    $$("[data-tab-jump]").forEach(el => {
      el.addEventListener("click", () => {
        const id = el.dataset.tabJump;
        if (!id) return;
  ...
  function setup() {
    ...
    on("readingFocus", "change", buildMirrorReading);

    on("buildMirror", "click", buildMirrorReading);
    on("copyMirror", "click", () => {
      const output = $("#mirrorOutput");
      if (output && navigator.clipboard) navigator.clipboard.writeText(output.textContent);
      toast("Mirror reading copied");
    });
