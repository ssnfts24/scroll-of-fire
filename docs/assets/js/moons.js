<script>
/* 13 Moons — Living Clock (advanced, DPR-aware, phase-tinted, v3.9) */
(function(){
  "use strict";
  ...
  const tzPick = $("#tzPick");
  const COMMON_TZ = ["UTC","America/Los_Angeles","America... "America/Los_Angeles","America/Denver","America/Chicago","America/New_York",
...
if (tzPick){
  ...
  on(tzPick,"change",()=>{
    ...
    writeURL(); updateAll(true); buildYearMap();
  }, {passive:true});
}
...
function hydrateLore(idx){
  const data = findMoonData(idx);
  if (!data) return;
  ...
  if (loreHebrew) loreHebrew.textContent = data.hebrew || "";
  if (lorePsalm)  lorePsalm.textContent  = data.psalm || "";
  if (loreCodex)  loreCodex.href         = data.codex || "#";
  if (loreEssay)  loreEssay.href         = data.essay || "#";
  if (loreSigil)  { loreSigil.src = data.sigil || ""; loreSigil.alt = `Sigil — ${MOONS[idx-1]}`; }

  if (practiceList && Array.isArray(data.practices)){
    practiceList.innerHTML = data.practices.map(p=>`<li>${p}</li>`).join("");
  }
  if (mediaImage && data.media?.image){
    mediaImage.src = data.media.image;
    mediaImage.alt = data.media?.alt || `${MOONS[idx-1]} — media`;
  }
  if (mediaCaption) mediaCaption.textContent = data.media?.caption || "";
  if (mediaAudio){
    if (data.media?.audio){ mediaAudio.src = data.media.audio; mediaAudio.style.display="block"; }
    else { mediaAudio.removeAttribute("src"); mediaAudio.style.display="none"; }
  }

  // Per-moon note restore
  const note = localStorage.getItem(`sof_moon_note_${idx}`) || "";
  const noteEl = $("#noteText");
  if (noteEl) noteEl.value = note;
}

/* ===================== Share image (badge snapshot) ===================== */
async function shareBadge(){
  try{
    const badge = $(".moon-badge");
    if (!badge) return false;

    // Render a lightweight canvas summary instead of DOM raster (no html2canvas)
    const w=700, h=240, dpr=2;
    const c=document.createElement("canvas"); c.width=w*dpr; c.height=h*dpr; c.style.width=w+"px"; c.style.height=h+"px";
    const k=c.getContext("2d"); k.scale(dpr,dpr);

    // Background
    const bg=k.createLinearGradient(0,0,0,h);
    bg.addColorStop(0,"#0a0e15"); bg.addColorStop(1,"#06080d");
    k.fillStyle=bg; k.fillRect(0,0,w,h);

    // Accents
    const styles = getComputedStyle(document.documentElement);
    const c1 = styles.getPropertyValue("--moon-accent").trim() || "#7af3ff";
    const c2 = styles.getPropertyValue("--moon-accent-2").trim() || "#f3c97a";

    // Ring
    const cx=120, cy=h/2, R=64;
    k.beginPath(); k.arc(cx,cy,R,0,Math.PI*2);
    k.strokeStyle="#1f2533"; k.lineWidth=10; k.stroke();

    // Arc
    const arc = $("#moonArc");
    let ratio = 0.0;
    if (arc){
      const d = arc.getAttribute("stroke-dasharray")||"0 316";
      const a = +d.split(" ")[0]||0;
      ratio = a/316;
    }
    k.beginPath(); k.arc(cx,cy,R,-Math.PI/2,-Math.PI/2 + ratio*2*Math.PI);
    const grad=k.createLinearGradient(cx-R,cy,cx+R,cy); grad.addColorStop(0,c1); grad.addColorStop(1,c2);
    k.strokeStyle=grad; k.lineWidth=10; k.lineCap="round"; k.stroke();

    // Text
    const name = $("#moonName")?.textContent || "—";
    const essence = $("#moonEssence")?.textContent || "—";
    const day = $("#dayInMoon")?.textContent || "—";
    k.font="700 24px Inter, system-ui, sans-serif"; k.fillStyle="#e9eef6"; k.fillText(name, 220, 96);
    k.font="600 14px Inter, system-ui, sans-serif"; k.fillStyle="#9aa4be"; k.fillText(essence, 220, 122);
    k.font="800 18px Inter, system-ui, sans-serif"; k.fillStyle="#cfefff"; k.fillText(`Day ${day} of 28`, 220, 152);

    const blob = await new Promise(res=> c.toBlob(res, "image/png", 0.95));
    if (!blob) return false;

    if (navigator.canShare && navigator.canShare({ files:[new File([blob], "13-moons.png", {type:"image/png"})] })){
      await navigator.share({
        title: "13 Moons — today",
        text: name,
        files:[ new File([blob], "13-moons.png", {type:"image/png"}) ]
      });
    } else {
    ...
    console.warn("shareBadge failed", e);
    return false;
  }
}
...
/* Hash deep-linking: #moon-4 or #moon-4-day-13 */
function applyHashIfAny(){
  const h = (location.hash||"").toLowerCase();
  const m = h.match(/#moon-(\d+)(?:-day-(\d+))?/);
  if (!m) return false;
  const moonN = clamp(parseInt(m[1],10)||1, 1, 13);
  const dayN  = clamp(parseInt(m[2],10)||1, 1, 28);
  const tz = tzPick?.value || "UTC";
  const anchor = startOfYear13(new Date(), tz);
  const spans  = yearMoonSpans(anchor, tz);
  const target = spans[moonN-1];
  if (!target) return false;
  // compute the desired day date
  let d = new Date(target.start);
  d = addDaysSkippingSpecials(d, dayN-1, tz);
  if (datePick) datePick.value = dateInTZ(d, tz).toISOString().slice(0,10);
  // keep current hour
  writeURL();
  return true;
}
...
// Auto-contrast for mini moonbar text (letters stay readable on bright accents)
const parseColor = (s) => {
  if (!s) return null;
  s=s.trim();
  if (s.startsWith("#")){
...
async function updateAll(forceTween=false){
  try{
    const tz=tzPick?.value || "UTC";
    const base=new Date();
    ...
    if(m13.special){
      ...
      // Lore hydrate (async, only when available)
      await loadMoonData();
      hydrateLore(m13.moon);

      // Per-moon note key on the chip for external hooks
      if (moonChip) moonChip.dataset.noteKey = `sof_moon_note_${m13.moon}`;
    }
...
// Share image button (optional)
on(btnShareImage,"click", async ()=>{
  const ok = await shareBadge();
  if (!ok){
    const t=document.createElement("div"); t.className="toast"; t.textContent="Couldn’t share image";
    document.body.appendChild(t); requestAnimationFrame(()=>t.style.opacity=1);
    setTimeout(()=>{t.style.opacity=0; setTimeout(()=>t.remove(),250);},1800);
  }
});

// Save per-moon note (optional)
on(btnSaveNote,"click", ()=>{
  const noteEl = $("#noteText");
  const idx = $("#moonChip")?.dataset?.idx;
  if (!noteEl || !idx) return;
  localStorage.setItem(`sof_moon_note_${idx}`, noteEl.value||"");
...
// React to hash changes at runtime
on(window, "hashchange", ()=>{
  if (applyHashIfAny()) updateAll(true);
}, {passive:true});
...
try{
  readURL();
  // If there is a hash targeting a moon, apply before initial render
  applyHashIfAny();
