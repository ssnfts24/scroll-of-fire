(function(){
  ...
  function makeICS(spans, tz, titleYear){
    const lines = [
      "BEGIN:VCALENDAR","VERSION:2.0","PRODID:-//Scroll of Fire//13 Moon//EN","CALSCALE:GREGORIAN","METHOD:PUBLISH"
    ];
    spans.forEach((s,i)=>{
  ...
  function weekAndDow(day1to28){
    const week = Math.floor((day1to28-1)/7)+1;     // 1..4
    const dow  = ((day1to28-1) % 7) + 1;          // 1..7
    return {week, dow};
  }
  ...
  function readURL(){
    ...
    if(z && tzPick){ tzPick.value=z; localStorage.setItem(TZ_KEY,z); }
  ...
  // ICS url cleanup
  let lastICSUrl=null;
  function setICSUrl(url){ if (lastICSUrl) URL.revokeObjectURL(lastICSUrl); lastICSUrl = url; }
  addEventListener('pagehide', ()=>{ if (lastICSUrl) URL.revokeObjectURL(lastICSUrl); }, {passive:true});
  ...
  function updateAll(){
    ...
    if(m13.special){
      ...
      if (weekDots){
        Array.from(weekDots.children).forEach(n => n.classList.remove('on'));
        weekDots.setAttribute('aria-label', `No week/day (special day)`);
      }
    } else {
    ...
    // Numerology + YHWH chip resonance
  ...
  // ICS refresh trigger
  on($("#regenICS"), "click", ()=>{ updateAll(); toast("Calendar refreshed"); });

  // Live clock + rollover in TZ
  setInterval(()=>{
