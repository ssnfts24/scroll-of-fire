...
test('B7.51 gives all 13 Pattern Moons distinct bodies plus one GPU number atlas', () => {
  const src = read('docs/assets/js/sphere/living-time-sphere-renderer-3d.js');
  assert.match(src, /MOON_IDENTITY_COLORS = Object\.freeze\(\[/);
  assert.match(src, /MOON_IDENTITY_SHAPES = Object\.freeze\(\[/);
  assert.match(src, /new _THREE\.InstancedMesh\(geometry, material, 13\)/);
  assert.match(src, /function _buildMoonNumberField\(/);
  assert.match(src, /new _THREE\.Points\(geometry, material\)/);
  assert.match(src, /aTile/);
  assert.match(src, /moonIdentityNumbers/);
  assert.match(src, /ctx\.fillText\(String\(n\)/);
});
...
test('B7.51 mobile calendar disclosure shrinks to one Moon in motion and three when settled', () => {
  const src = read('docs/assets/js/sphere/living-time-sphere-renderer-3d.js');
  assert.match(src, /CAMERA_CALENDAR_HALF_WINDOW_DESKTOP = 2/);
  assert.match(src, /return _cameraGestureActive \? 0 : 1/);
  assert.match(src, /const halfWindow = _calendarDisclosureHalfWindow\(\)/);
  assert.match(src, /_calendarDisclosureDirty = true;[\s\S]*interaction changes the mobile disclosure aperture/);
});
...
test('B7.51/B7.52 cache-busts the full Sphere while Moons remains ambient-only', () => {
  ...
  assert.match(version, /APP_VERSION = "2026\.08\.19\.(?:51|52)"/);
});
